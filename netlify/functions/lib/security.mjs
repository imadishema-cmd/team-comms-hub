...
export function cleanupAuth(auth) {
  ...
  for (const [token, session] of Object.entries(auth.sessions || {})) {
    if (new Date(session.expiresAt).getTime() < timestamp) {
  ...
  for (const [token, record] of Object.entries(
    ...
    if (new Date(record.expiresAt).getTime() < timestamp) {
      delete auth.verificationTokens[token];
    }
  }
  ...
  for (const [token, record] of Object.entries(
    ...
    if (new Date(record.expiresAt).getTime() < timestamp) {
      delete auth.mfaChallenges[token];
    }
  }
  ...
  for (const [token, record] of Object.entries(
    ...
    if (new Date(record.expiresAt).getTime() < timestamp) {
      delete auth.quizSessions[token];
    }
  }
}
...
export async function enforceRateLimit(
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    const stored = await store.getWithMetadata(key, {
      type: 'json',
      consistency: 'strong',
    });
    ...
    if (record.lockedUntil > timestamp) {
      const seconds = Math.ceil(
        (record.lockedUntil - timestamp) / 1000,
      );
    ...
    if (record.count > limit && lockoutMs) {
      record.lockedUntil = timestamp + lockoutMs;
    }
    ...
    if (record.count > limit) {
      const seconds = lockoutMs
        ? Math.ceil(lockoutMs / 1000)
        : Math.ceil(
            (
              windowMs -
              (timestamp - record.windowStartedAt)
            ) / 1000,
          );
  ...
  throw Object.assign(
    new Error('Rate-limit check conflicted. Please retry.'),
    { status: 409 },
  );
}
