...
export const contentStore = () =>
  getStore({ name: NEW_CONTENT_STORE, consistency: 'strong' });

export const authStore = () =>
  getStore({ name: NEW_AUTH_STORE, consistency: 'strong' });

export const progressStore = () =>
  getStore({ name: NEW_PROGRESS_STORE, consistency: 'strong' });

export const fileStore = () =>
  getStore({ name: NEW_FILE_STORE, consistency: 'strong' });

export const oldFileStore = () =>
  getStore({ name: OLD_FILE_STORE, consistency: 'strong' });

export const rateStore = () =>
  getStore({ name: RATE_STORE, consistency: 'strong' });

export const reminderStore = () =>
  getStore({ name: REMINDER_STORE, consistency: 'strong' });
...
export async function ensureMigrated() {
  ...
  const oldContent = getStore({
    name: OLD_CONTENT_STORE,
    consistency: 'strong',
  });

  const oldWorkspace =
    (await oldContent.get('workspace', {
      type: 'json',
      consistency: 'strong',
    })) || {};
  ...
  for (const name of COLLECTIONS) {
    ...
    if (existing === null) {
      await store.setJSON(
        name,
        upgradeCollection(name, oldWorkspace[name] || []),
        { onlyIfNew: true },
      );
    }
  }
  ...
  const existingAuth = await newAuth.get('auth', {
    type: 'json',
    consistency: 'strong',
  });

  if (existingAuth === null) {
    const oldAuth = getStore({
      name: OLD_AUTH_STORE,
      consistency: 'strong',
    });

    const oldAuthData =
      (await oldAuth.get('auth', {
        type: 'json',
        consistency: 'strong',
      })) || {};

    await newAuth.setJSON('auth', upgradeAuth(oldAuthData), {
      onlyIfNew: true,
    });
...
export async function readCollections(names = COLLECTIONS) {
  ...
  const values = await Promise.all(
    selected.map((name) => readCollection(name)),
  );

  return Object.fromEntries(
    selected.map((name, index) => [name, values[index]]),
  );
}
...
async function mutateMeta(collection) {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    const record = await store.getWithMetadata('meta', {
      type: 'json',
      consistency: 'strong',
    });
    ...
    meta.revisions[collection] =
      Number(meta.revisions[collection] || 0) + 1;
  ...
  throw Object.assign(
    new Error('The workspace changed at the same time. Please retry.'),
    { status: 409 },
  );
}
...
export async function mutateCollection(name, mutator) {
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    const record = await store.getWithMetadata(name, {
      type: 'json',
      consistency: 'strong',
    });
...
export async function mutateAuth(mutator) {
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    const record = await store.getWithMetadata('auth', {
      type: 'json',
      consistency: 'strong',
    });
  ...
  throw Object.assign(
    new Error('Account data changed at the same time. Please retry.'),
    { status: 409 },
  );
}
...
export async function readProgress(userId) {
  ...
  const value = await progressStore().get(`user:${userId}`, {
    type: 'json',
    consistency: 'strong',
  });
...
export async function mutateProgress(userId, mutator) {
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    const record = await store.getWithMetadata(key, {
      type: 'json',
      consistency: 'strong',
    });
  ...
  throw Object.assign(
    new Error(
      'Learning progress changed at the same time. Please retry.',
    ),
    { status: 409 },
  );
}
